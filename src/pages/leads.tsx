...
export function Leads() {
  ...
  var startDate: Date | undefined | null = task?.card[0].data_criacao
    ? new Date(task?.card[0].data_criacao)
    : null;
  var daysInStep = startDate
    ? Math.floor(
  ...
  const alertNotAllFields = () => {
    ...
    if (
      validatedFields.data_prevista ||
      validatedFields.titulo ||
      validatedFields.valor_estimado
    ) {
      setActiveTab(1);
      return;
    }
  ...
  const carregaMunicipios = async (CodEstado?: string) => {
    let endpointMunicipios = "/leads/municipios";
    if (CodEstado) {
      endpointMunicipios = `/leads/municipios?estado=${CodEstado}`;
    }

    const municipios = await ajax({
      method: "GET",
      endpoint: endpointMunicipios,
      data: null,
    });

    if (municipios.status == "success") {
      setMunicipios(municipios.data);
      return;
    }
    if (municipios.status == "error") {
      toast.error(municipios.message);
      return;
    }
  };
  /*
            ...
            {loadingLead ? (
              ...
              <div className="w-11/12 flex self-center justify-between ">
                ...
                <div
                  ...
                  data-tooltip-content="Ao selecionar esse botão, será criado uma oportunidade para esse cliente e o potencial será alterado para uma oportunidade em Qualificação!"
                >
                  <GrnBtn
                    ...
                    customCss={
                      isValidated
                        ? ""
                        : "bg-slate-500 hover:scale-100 hover:bg-slate-500"
                    }
                    form="addLead"
                    type="submit"
                    median={true}
                    big={false}
                    nomeBtn="Transformar em oportunidade!"
                    icon={
                      isValidated ? (
                        <MdUpload size={30} />
                      ) : (
                        <RiProhibitedLine size={30} />
                      )
                    }
                    onClick={
                      isValidated ? onSubmitRegisterLead : alertNotAllFields
                    }
                  />
                </div>
              </div>
            ...
            {loadingLead ? (
              ...
              <div className="flex w-11/12 self-center justify-self-center gap-2 ">
                ...
                >
                  ...
                  <div className="triangle-right-title-white absolute"></div>{" "}
                  <div
                    className={`w-full bg-custom-gray flex items-center justify-center text-white font-semibold`}
                  ></div>
                  <div className="triangle-right-title-grey"></div>
                </div>
                <div
                  data-tooltip-id="tooltip-4"
                ...
                >
                  {" "}
                  <div className="triangle-right-title-white absolute"></div>{" "}
                  <div className={`w-full bg-custom-gray`}> </div>
                  <div className="triangle-right-title-grey"></div>
                </div>
                <div
                  data-tooltip-id="tooltip-4"
          ...
          <div className="p-2 max-w-full bg-gray-100">
            {loadingLead ? (
              ...
              <div className=" max-w-full rounded-md h-auto bg-white shadow-lg mb-6 flex gap-4 items-center border">
                <div
                  ...
                  } flex items-center gap-4 cursor-pointer px-6`}
                >
                  <ImProfile size={30} />
            ...
            <div className=" p-4 max-w-full h-full bg-white shadow-lg rounded-md ">
              {loadingLead ? (
                ...
                <GeralLead task={task} errors={validatedFields} />
              ) : activeTab == 2 ? (
                <DadosCliente
                  errorsRegister={validatedFields}
                  task={task}
                  estados={estados}
                  municipios={municipios}
                  selecionaEstado={carregaMunicipios}
                />
              ) : activeTab == 3 ? (
                <NotesLead task={task} />
              ) : (
