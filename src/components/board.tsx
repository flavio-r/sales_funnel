    ...
    async function addTaskToDestination(destinationId: string, taskId: string, index: number) {
        if (destinationId == "7" || destinationId == "6") {
                ...
                if ( response.status == 'success' ) {
                    if (destinationId == "7") {
                        toast("Oportunidade Perdida", {icon: '😭', className: "bg-red-500 text-white font-semibold"});
                        return;
                    }   
                    if (destinationId == "6") {
                        setIsExploding(true);
                        toast("Oportunidade Ganha!", { icon: '🎉', className: "bg-green-500 text-white font-semibold"});
                        setTimeout(() => {
