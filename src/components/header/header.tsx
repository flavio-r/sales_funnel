...
export function Header({ setSearch, setFilters, setGestor, Filters }: header) {
  ...
  const handleMostrarModal = () => {
    ...
    // Remove parâmetros da URL quando fechar o modal
    if (mostrarAdicionar) {
  ...
  const checaIsGerente = async () => {
    const response = await ajax({
      method: "GET",
      endpoint: "/gestoria/checaGerente",
      data: null,
    });
  ...
  const checaIsInterno = async () => {
    const response = await ajax({
      method: "GET",
      endpoint: "/gestoriaInterna/checaInterno",
      data: null,
    });
    if (response.status == "success") {
  ...
  const checaIsSupervisoraInterna = async () => {
    const response = await ajax({
      method: "GET",
      endpoint: "/superGestoriaInterna/isSupervisor",
      data: null,
    });
    if (response.status == "success") {
  ...
  const checaIsVisualizador = async () => {
    const response = await ajax({
      method: "GET",
      endpoint: "/visualizadores/isVisualizador",
      data: null,
    });
    if (response.status == "success") {
  ...
  return (
    <>
      <AddOportunity
        atualizaEstadoModal={handleMostrarModal}
      ...
      <div className="headerHeight flex shadow-md w-full items-center box-border relative justify-between">
        ...
        <div className="flex w-auto gap-8 items-center relative self-center">
          ...
          {localSearch == "" ? (
            ...
            <div
              className="absolute mr-4 right-52 mt-1"
