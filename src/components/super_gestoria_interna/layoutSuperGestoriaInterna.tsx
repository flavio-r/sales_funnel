...
export function LayoutSuperGestoriaInterna() {
  ...
  const updateExternos = async (supervisionados: any) => {
    try {
      await ajax({
        method: "PATCH",
        endpoint: "/atualizarSupervisionados",
    ...
    <>
      <SearchContextGestoria.Provider
        value={{
          ...
          gerenciados: user.supervisionados,
          allGerenciados: allGerenciados,
          indicadoresContext: indicadores,
          alteraIndicadores: setIndicadores,
        }}
      >
        <HeaderSuperGestoriaInterna
          Supervisionados={user.supervisionados}
          Filters={user.filtros.filtros}
          setSearch={setSearch}
          setFilters={updateFilters}
          setExternosContext={updateExternos}
          setAllExternosContext={setAllGerenciados}
        />
        <Outlet />
      </SearchContextGestoria.Provider>
    </>
