...
export function HeaderSuperGestoriaInterna({
  setSearch,
  setFilters,
  setExternosContext,
  setAllExternosContext,
  Filters,
  Supervisionados,
}: header) {
  ...
  return (
    <>
      <AddOportunity
        atualizaEstadoModal={() => setMostrarAdicionar(!mostrarAdicionar)}
      ...
      <div className="headerHeight flex shadow-md w-full items-center box-border relative justify-between">
        ...
        <div className="flex w-auto gap-8 items-center relative self-center">
          ...
          <input
            disabled={
              pathname.includes("/opportunity") || pathname.includes("/leads")
                ? true
                : false
            }
            type="text"
            ...
            className={
              pathname.includes("/opportunity") || pathname.includes("/leads")
                ? styles.searchDesativado
                : styles.search
            }
            onChange={(e) => handleSearch(e.target.value)}
            placeholder="Busque por Oportunidades"
          />
          {localSearch == "" ? (
            ""
          ) : (
            <div
              className="absolute mr-4 right-0 mt-1"
          ...
          <div className="flex gap-4 mr-4 ml-4">
            <div className="flex flex-col items-center justify-center">
              <p className="m-0 text-xs font-semibold text-green-500 ">
                Ganhos (30 dias)
              </p>
              <button className=" hover:scale-105 h-9 w-24 customGreenBorder outline-none bg-white rounded-md font-semibold text-green-500 cursor-pointer hover:bg-green-500 hover:text-white transition-all duration-300 ">
                {indicadores.ganhos || indicadores.ganhos == 0 || (
                  <ImSpinner8 className="animate-spin mt-1" />
                )}
              </button>
            </div>
            <div className="flex flex-col items-center justify-center">
              ...
              <button className=" hover:scale-105 h-9 w-24 customRedBorder outline-none bg-white rounded-md font-semibold text-red-500 cursor-pointer hover:bg-red-500 hover:text-white transition-all duration-300 ">
                {indicadores.perdidos || indicadores.perdidos == 0 || (
                  <ImSpinner8 className="animate-spin mt-1" />
                )}
              </button>
            </div>
            <div className="flex flex-col items-center justify-center">
              ...
              <button className=" hover:scale-105 h-9 flex items-center justify-center customBorder outline-none bg-white rounded-md font-semibold text-black cursor-pointer hover:bg-black hover:text-white transition-all duration-300 ">
                {indicadores.valorTotal ? (
                  new Intl.NumberFormat("pt-BR", {
                    style: "currency",
                    currency: "BRL",
                  }).format(indicadores.valorTotal)
                ) : (
        ...
        <div className=" relative flex justify-center mr-2 ">
          ...
          <div
            className={` ${
              showVendors ? "" : "hidden"
            } customBorder mt-14  rounded-md box-border absolute z-50 bg-white flex flex-col `}
          >
            {supervisionados ? (
              supervisionados.map((interna: supervisionado, index) => {
                return (
                  <>
                    ...
                    >
                      <p className="m-0 box-border whitespace-nowrap">
                        {interna.NomeInterna}
                      </p>
                      <div className="w-full h-full flex justify-end items-center ">
                        <input
                          checked={interna.Selecionada}
                          onChange={() => handleInterna(interna)}
                          type="checkbox"
                          className=" w-16 h-4 p-2 transition-all duration-300 hover:scale-105 "
                        />
                        <button
                          onClick={() => toggleDropdown(index)}
                          className="ml-2 focus:outline-none bg-transparent flex items-center justify-center rounded-md p-1 border-none  cursor-n-resize "
                        >
                    ...
                    {openedDropdowns.includes(index) && (
                      <div id="externos" className="mt-2">
                        {interna.externos ? (
                          interna.externos.map(
                            (externo: externoSupervisionado) => (
                              <label
                                onClick={(e) => e.stopPropagation()}
                                className=" flex px-4 py-3 w-full box-border justify-between transition-all duration-300 hover:bg-black hover:bg-opacity-10 cursor-pointer "
                              >
                                <p className="m-0 ml-8 box-border whitespace-nowrap">
                                  {externo.NomeExterno}
                                </p>
                                <input
                                  checked={externo.Selecionado}
                                  onChange={() => handleExternos(externo)}
                                  type="checkbox"
            ...
            <div className="w-full flex items-center gap-2 justify-center px-2 py-1 box-border">
              <GrnBtn
                onClick={() => atualizaExternosContext()}
                customCss="mt-2 mb-2 w-full self-center"
                nomeBtn="Aplicar Supervisionados"
                type="submit"
            ...
            <div className="w-full flex items-center gap-2 justify-center px-2 py-1 box-border">
              <GrnBtn
                onClick={() => myOpportunities()}
                customCss="mt-2 mb-2 w-full self-center bg-green-700 "
                nomeBtn="Minhas oportunidades"
                type="submit"
            ...
            <div className="w-full flex items-center gap-2 justify-center p-2 box-border">
              <GrnBtn
                customCss="w-6/12 whitespace-nowrap"
                onClick={() => selectAll()}
                nomeBtn="Selecionar todos"
                icon={<BiSelectMultiple />}
              />
              <GrnBtn
                customCss="w-6/12 whitespace-nowrap bg-red-500 hover:bg-red-600"
                onClick={() => deSelectAll()}
                nomeBtn="Remover todos"
        ...
        <div className="justify-self-end flex items-center justify-center gap-8 mr-6 h-full">
          {pathname.includes("/opportunity") ? null : (
            <WhiteBtn
          ...
          <Filter
            baseFilters={Filters}
            showFilters={mostrarFiltro}
            handleFilters={HandleSetFilters}
            fecharFiltro={handleFiltro}
          />
          <div>
            <CgProfile
              size={33}
              onClick={() => handleProfile()}
              className="mr-8 mt-1.5 hover:scale-105 transition-all duration-500 cursor-pointer"
            />
