...
export function SuperChangeOwnerInterna({atualizaEstadoModal, mostrarModal, idTask, currentOwner} : {atualizaEstadoModal: () => void, mostrarModal: boolean, idTask: number, currentOwner: string}) {
    ...
    const onSubmit = async (data: any) => {
        ...
        const response = await ajax({method: "POST", endpoint: "/superGestoriaInterna/alterarResponsavel", data: {CodigoVendedor: data.owner, IdTask: idTask}});
        toast.dismiss();
        setIsLoading(false);
        ...
        <div className={` ${isOpen ? "" : "hidden"} modalAdd fixed top-0 left-0 w-full h-full bg-opacity-50 bg-black overflow-auto z-50 flex items-center justify-center box-border`}>
            <div className={`bg-white ${isOpen ? "slide-up" : "slide-down"}   w-3/6 rounded-md shadow-md flex flex-col`} >
                ...
                <form id="alterTaskOwner" action="" onSubmit={handleSubmit(onSubmit)} className="flex h-9/12 w-full px-8 py-4 box-border mb-4  justify-between gap-4">
                    <SelectDadosSuperGestoriaInterna preOwner={currentOwner} tipo="owner" externos={allGerenciados}  placeholder="Responsável" name="owner" register={register}  />
                </form>
