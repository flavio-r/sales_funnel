...
import { TaskContextGestoria } from "./boardSuperGestoriaInterna";
...
export function TaskSuperGestoriaInterna({ task, index }: taskProps) {
  ...
  const {
    ...
    alterSelectedTask,
    alterCurrentOwner,
  } = useContext(TaskContextGestoria);

  const handleOpenTask = (task: taskProps["task"]) => {
    navigate(`/opportunity/${task.Id}?origem=superGestoriaInterna`);
  };
  ...
  const handleHighlightTask = async () => {
    if (!isHighlighted) {
      ...
      const response = await ajax({
        method: "POST",
        endpoint: "/gestoria/highlightTask",
        data: { taskId: task.Id },
      });
    ...
    if (isHighlighted) {
      ...
      const response = await ajax({
        method: "POST",
        endpoint: "/gestoria/unhighlightTask",
        data: { taskId: task.Id },
      });
      if (response.status == "error") {
  ...
  const changeHeatLevel = async (e: any) => {
    ...
    const response = await ajax({
      method: "POST",
      endpoint: "/task/updateHeatLevel",
      data: { taskId, heatLevel: heatLevel },
    });
    if (response.status == "success") {
      //
    } else {
    ...
    <>
      <Draggable draggableId={task.Id.toString()} key={task.Id} index={index}>
        {(provided: any, snapshot: any) => {
          return (
            <div
              className={`z-40 hover:z-50 w-full h-auto px-2 scaleCustom py-2 rounded-md ${
                isInDrag ? "bg-slate-200" : "bg-white"
              } items-center flex flex-row border border-black shadow-custom-shadow`}
            ...
            >
              <div className="w-11/12">
                ...
                <div className="flex items-center gap-2 mt-3">
                  <div
                    onClick={() => handleChangeTaskOwner()}
                    className=" gap-1 whitespace-nowrap flex items-center customBorder rounded-md px-1 cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-sm"
                  >
                    <CgProfile />
                    <p className="m-0"> {task.firstName} </p>
                  </div>
                  <div className="m-0 text-xs font-semibold flex gap-1 items-center">
                    <p className="m-0"> R$ {formatedPrice} </p> <LuDot />{" "}
                    <span className={dateColor}> {formatedDate} </span>{" "}
                  </div>
                </div>
              </div>
              ...
              <div className="h-full flex flex-col justify-between ml-3">
                ...
                >
                  <BsExclamationCircleFill
                    ...
                    } corDestaque  cursor-pointer transition-all duration-300 `}
                  />{" "}
                  <BsExclamationCircle
                    size={22}
                    className={` ${
                ...
                {isInDrag ? (
                  <ImSpinner8 className="animate-spin" size={22} />
                ) : (
                  <FaCirclePlus
                    onClick={() => handleOpenTask(task)}
                    size={22}
                    className=" hover:rotate-90 cursor-pointer transition-all duration-300 hover:scale-125 p-1  "
                  />
                )}
                <div
                  className=" p-1 py-2 border flex items-center justify-center cursor-pointer"
                  onClick={() => handleHeatLevel()}
                >
                  {heatColor === "fireIcon" ? (
                  ...
                  <div
                    ...
                    className={`${
                      showHeatModal ? "absolute" : "hidden"
                    } z-custom py-2 flex transition-all duration-300 flex-col encaixarModalHeat w-44 h-40 bg-white rounded-md borderCustom shadow-md`}
                  >
                    <div
                      data-heatlevel={-1}
                      className="flex h-16 items-center justify-between px-2 cursor-pointer hover:bg-slate-200"
                    >
                    ...
                    <div
                      data-heatlevel={1}
                      className="flex h-16 items-center justify-between px-2 hover:bg-slate-200"
                    >
