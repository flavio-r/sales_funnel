...
import { gerenciado } from "./headerVisualizadores";
interface input {
  placeholder: string;
  name: string;
...
export function SelectDadosVisualizadores({
  tipo,
  funcaoAoMudar,
  requiredDefault,
  register,
      ...
      <div className="flex items-center w-full ">
        <select
          ...
          {...(funcaoAoMudar
            ...
            : { ...register(name, rules) })}
          className={` corPlaceholder text-black fontePadrao font-semibold colocarBorda w-full h-10 rounded-md border-none text-lg  ${
            icon ? "pl-9" : "pr-6 pl-2"
          ...
          {tipo == "owner" ? (
            <>
              {gerenciados.map((gerenciado: gerenciado) => {
                ...
                return (
                  <option
                    selected={cleanName == preOwner ? true : false}
                    value={gerenciado.CodigoVendedor}
                  >
                    {cleanName}
                  </option>
                );
