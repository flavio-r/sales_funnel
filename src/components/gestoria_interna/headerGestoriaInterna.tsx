...
export function HeaderGestoriaInterna({
  setSearch,
  setFilters,
  setExternosContext,
  setAllGerenciadosContext,
  setAllVendorsContext,
  Filters,
  Externos,
}: header) {
  ...
  const carregaGerenciados = async () => {
    ...
    const response = await ajax({
      method: "GET",
      endpoint: "/gestoriaInterna/externos",
      data: null,
    });
    ...
    if (response.status == "success") {
      ...
      adicionaSelecionadoGerenciados(data, Externos);
    }
  };
  ...
  const handleGerenciados = (gerenciado: externo) => {
    const index = externos.findIndex(
      (item) => item.CodigoExterno == gerenciado.CodigoExterno
    );
  ...
  const atualizaGerenciadosContext = () => {
    var gerenciadosFiltrados = externos.filter(
      (externo) => externo.Selecionado == true
    );
          ...
          <div className="flex gap-4 mr-4 ml-4">
            ...
            <div className="flex flex-col items-center justify-center">
              ...
              <button className=" hover:scale-105 h-9 flex items-center justify-center customBorder outline-none bg-white rounded-md font-semibold text-black cursor-pointer hover:bg-black hover:text-white transition-all duration-300 ">
                {indicadores.valorTotal == null ||
                indicadores.valorTotal == undefined ? (
