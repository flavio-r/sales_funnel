...
import { ColumnGestoriaInterna } from "./columnGestoriaInterna";
...
export function BoardGestoriaInterna() {
  const { searchValue, filters, gerenciados, alteraIndicadores } =
  ...
  const [indicadoresState, setIndicadoresState] = useState<any>({});
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [isFirstLoadFilter, setIsFirstLoadFilter] = useState(true);
  const [isFirstLoadGerenciados, setIsFirstLoadGerenciados] = useState(true);
  ...
  async function addTaskToDestination(
    ...
    if (destinationId == "7" || destinationId == "6") {
      ...
      const response = await ajax({
        method: "PATCH",
        endpoint: "/task/ganhouPerdeu",
        data: { taskId: taskId, type: destinationId },
      });
      toast.dismiss();
      if (!response) {
    ...
    await ajax({
      method: "PATCH",
      endpoint: "/task/atualizar",
      data: { stageKey: stageKeySap, taskId: taskId },
    });
  ...
  const handleDragEnd = async (result: any) => {
    ...
    if (source.droppableId == "-1" || destination.droppableId == "-1") {
      if (destination.droppableId == "7") {
        ...
        const response = await ajax({
          method: "POST",
          endpoint: "/lead/desativar",
          data: { id_lead: id_card },
        });
        if (!response) {
  ...
  const carregaTasksSAP = async (filter: string = "") => {
    const response = await ajax({
      method: "POST",
      endpoint: "/gestoriaInterna/tasks",
    ...
    // Evitar re-renders desnecessários dos indicadores
    if (
      JSON.stringify(response.indicadores) !== JSON.stringify(indicadoresState)
    ) {
      setIndicadoresState(response.indicadores);
    ...
    const allTasks = contatoTasks.concat(
      leadTasks,
      diagnosticoTasks,
      testeTasks,
      negociacaoTasks
  ...
  useEffect(() => {
    ...
    if (isFirstLoadGerenciados) {
      setIsFirstLoadGerenciados(false);
      return;
    }
    ...
    <>
      ...
      <TaskContextGestoria.Provider
        value={{
          ...
          alterModalState: handleChangeModalState,
          alterLeadModalState: handleLeadModalState,
          alterSelectedTask: setSelectedTaskId,
          alterSelectedLead: setSelectedLeadId,
          alterCurrentOwner: setCurrentOwner,
        }}
      >
        <>{isExploding && <ConfettiExplosion {...confettiProps} />}</>

        <DragDropContext
          useDrag={handleGanhouPerdeu}
          ...
          <div className="flex mt-6 bg-white box-border max-w-full h-auto">
            {
              <ChangeOwnerInterna
                mostrarModal={isOpen}
                atualizaEstadoModal={handleChangeModalState}
                idTask={selectedTaskId}
                currentOwner={currentOwner}
              />
            }
            {
              <ModalChangeLeadOwner
                mostrarModal={isLeadOpen}
                atualizaEstadoModal={handleLeadModalState}
                idLead={selectedLeadId.toString()}
                currentOwner={currentLeadOwner}
              />
            }
            <ColumnGestoriaInterna
              title="Qualificação"
              tasks={lead}
              leads={leadMysql}
              id="1"
            ...
            <ColumnGestoriaInterna
              ...
              id="3"
            />
            <ColumnGestoriaInterna title="Teste" tasks={teste} id="4" />
            <ColumnGestoriaInterna
