...
export function BoardGestoriaInterna() {
    ...
    const carregaTasksSAP = async (filter: string = "") => {
        const response = await ajax({ method: 'POST', endpoint: '/gestoriaInterna/tasks', data: {filter: filter, filtros: filters, externos: gerenciados} });
        if (!response) {
            return false;
        }
        if (response.status === 'error') {
    ...
    return (
        ...
        <DragDropContext useDrag={handleGanhouPerdeu} onDragEnd={handleDragEnd} onDragStart={handleDragStart}>
            ...
            <ColumnGestoriaInterna title="Proposta de Valor" tasks={negociacao} id="5"/>
