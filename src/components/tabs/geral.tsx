...
export function Geral({ task }: { task?: task }) {
  ...
  const [predictedDateFormatted, setPredictedDateFormatted] =
    useState<string>("");
  ...
  useEffect(() => {
    const startDate: Date = task?.StartDate
      ? new Date(task.StartDate)
      : new Date();
    const predictedDate: Date = task?.PredictedClosingDate
      ? new Date(task?.PredictedClosingDate)
      : new Date();
    ...
    var predictedDateFormatted2 = predictedDate
      ? `${predictedDate.getDate().toString().padStart(2, "0")}/${(
          predictedDate.getMonth() + 1
        )
          .toString()
          .padStart(2, "0")}/${predictedDate.getFullYear()}`
      : "";
      ...
      <div className="flex w-full h-full">
        <div className="w-1/2 flex flex-col ">
          <div className="ml-4 w-10/12 gap-6 self-center h-28 shadow-lg flex items-center justify-center rounded-md customBorder">
            ...
            <InputDados
              isChanged={changedFields.includes("nomeEmpresa")}
              wasChanged={handleFieldChanged}
              register={register}
              editable={true}
              preValue={`${
              ...
              customCss="widthCustom"
              name="nomeEmpresa"
            />
          </div>
          <div className="flex flex-col w-10/12 mt-6 self-center gap-4">
            <InputDados
              isChanged={changedFields.includes("titulo")}
              wasChanged={handleFieldChanged}
              register={register}
              editable={true}
              preValue={`${task?.OpportunityName}`}
              placeholder="Título"
              name="titulo"
            />
            <InputDados
              isChanged={changedFields.includes("valor")}
              wasChanged={handleFieldChanged}
              register={register}
              type="numero"
              editable={true}
              icon={<MdAttachMoney size={20} />}
              preValue={`${task?.MaxLocalTotal}`}
              placeholder="Valor Estimado"
              name="valor"
            />
            <InputDados
              isChanged={changedFields.includes("data")}
              wasChanged={handleFieldChanged}
              register={register}
              editable={false}
              preValue={`${startDateFormatted}`}
              placeholder="Data de início da oportunidade"
              name="data"
            />
            <InputDados
              isChanged={changedFields.includes("codigoCliente")}
              wasChanged={handleFieldChanged}
              register={register}
              editable={false}
              preValue={`${task?.CardCode}`}
              placeholder="Código do Cliente"
              name="codigoCliente"
            />
            <InputDados
              isChanged={changedFields.includes("diasEtapaAtual")}
              wasChanged={handleFieldChanged}
              register={register}
              editable={false}
              preValue={`${0}`}
              placeholder="Dias na etapa atual"
              name="diasEtapaAtual"
            />
            <InputDados
              isChanged={changedFields.includes("diaVisita")}
              wasChanged={handleFieldChanged}
              register={register}
              type="date"
              editable={true}
              preValue={`${
                predictedDateFormatted != null ? predictedDateFormatted : ""
              }`}
            ...
            <InputDados
              ...
              type="text"
              editable={false}
              preValue={task?.Criador.nomeCriador}
              placeholder="Criador da oportunidade"
              name="criador"
            />
          </div>
        </div>
        ...
        <div className="w-1/2 flex flex-col">
          <div className=" ml-4 w-10/12 gap-6 self-center h-28 shadow-lg flex items-center justify-center rounded-md customBorder">
            ...
            <InputDados
              ...
              isChanged={changedFields.includes("pessoaContato")}
              customCss="widthCustom"
              name="pessoaContato"
            />
          </div>
          <div className=" ml-4 self-center w-10/12 mt-6 flex flex-col">
            <div className="flex flex-col gap-3">
              <InputDados
                ...
                editable={true}
                preValue={`${task?.Phone1}`}
                isChanged={changedFields.includes("telefone")}
            ...
            <div className="flex flex-col gap-3 mt-28">
              <InputDados
                isChanged={changedFields.includes("rua")}
                wasChanged={handleFieldChanged}
                register={register}
                editable={false}
                preValue={task?.BPAddress.Street}
                placeholder="Rua"
                name="rua"
              />
              <InputDados
                isChanged={changedFields.includes("bairro")}
                wasChanged={handleFieldChanged}
                register={register}
                editable={false}
                preValue={task?.BPAddress.Block}
                placeholder="Bairro"
                name="bairro"
              />
              <InputDados
                isChanged={changedFields.includes("cidade")}
                wasChanged={handleFieldChanged}
                register={register}
                editable={false}
                preValue={task?.BPAddress.City}
                placeholder="Cidade"
                name="cidade"
              />
            </div>
      ...
      <div className="mt-10 flex w-full justify-end">
        <div>
          <GrnBtn
            form="formAttOp"
            type="submit"
            nomeBtn="Atualizar Oportunidade"
