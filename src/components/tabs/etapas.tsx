  ...
  function handeleEtapas() {
    ...
    etapas.forEach((etapa: any) => {
      ...
      const diffMilliseconds: number =
        closingDate.getTime() - startDate.getTime();
      var diasNaEtapa: number = Math.floor(
        diffMilliseconds / (1000 * 60 * 60 * 24)
      );
      const startDateFormatted: string = `${startDate
        .getDate()
        .toString()
        .padStart(2, "0")}/${(startDate.getMonth() + 1)
        .toString()
        .padStart(2, "0")}/${startDate.getFullYear()}`;
      const finishDateFormatted: string = `${closingDate
        .getDate()
        .toString()
        .padStart(2, "0")}/${(closingDate.getMonth() + 1)
        .toString()
        .padStart(2, "0")}/${closingDate.getFullYear()}`;
      if (diasNaEtapa == 0) {
        diasNaEtapa = 1;
      }
      var objToAdd = {
    ...
    <div className="flex flex-col">
      ...
      {etapas.map((etapa: any, index: number) => {
        return (
          <div
            className={` ${
              index != counter ? "hover:bg-white" : ""
            } transition-all hover:shadow-lg duration-500 flex items-center w-full rounded customBorder mb-6 ${
              index != counter ? "bg-custom-gray" : ""
            }  shadow-md px-4 py-2 justify-between box-border`}
          >
