...
export function Cotacoes({ task }: { task?: task }) {
    ...
    async function getQuotationsFromClient(CardCode: string) {
        ...
        const response = await ajax({ method: "GET", endpoint: `/cotacoes/${CardCode}`, data: null });
        ...
        <>
            <div className="  box-border" >
                <div className=" flex justify-between mb-2 items-end" >
                    <GrnBtn nomeBtn="Criar proposta de valor" onClick={() => redirectCriarProposta()} icon={<GoArrowUpRight />} customCss="h-10" />
                </div>
                    ...
                    {loadingCotacoes ? (
                        ...
                        Array.isArray(cotacoes) && cotacoes.length === 0 ? (
                            <div style={{ height: "80vh" }} className="w-full flex flex-col items-center justify-center">
                                ...
                                <p className="font-semibold text-xl">Esse cliente não possui cotações</p>
                            </div>
                        ) : (
                            Array.isArray(cotacoes) && cotacoes.map((cotacao: cotacao, index) => {
                                return (
                                    <Cotacao cotacao={cotacao} index={index} />
                                )
                            })
                        )
                    )}
