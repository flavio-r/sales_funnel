...
export function Notes({ task }: { task?: task }) {
  ...
  const handleSubmit = async () => {
    ...
    const response = await ajax({
      method: "PATCH",
      endpoint: "/task/atualizarNotas",
      data: { taskId: task?.SequentialNo, notas: quillValue },
    });
    if (!response) {
      toast.dismiss();
    ...
    <div className="w-full h-full flex flex-col">
      ...
      <div className="flex w-full mt-6">
        <textarea
          ...
          className={`myQuillBig text-xl customBorder rounded-md p-2 focus:outline-none shadow-md`}
        />
      </div>
    </div>
  );
