...
export function Atividades({ task }: { task?: task }) {
  ...
  const enviaTicket = async () => {
    ...
    const response = await ajax({
      method: "POST",
      endpoint: "/atividades/atividadesRapidas",
      data: dataQuickAction,
    });
    toast.dismiss();
    if (!response) {
  ...
  const carregaAtividades = async (filter: string) => {
    ...
    const response = await ajax({
      method: "POST",
      endpoint: "/atividades",
      data: { filter: filter, cardcode: task?.CardCode },
    });
    if (!response) {
      toast.dismiss();
  ...
  return (
    <>
      <ModalConteudo
        show={showModalConteudo}
        fechaModal={alterShow}
        atualizaModal={handleModalConteudoChange}
        atualizaDataModal={handleDataModalChange}
        titulo={tituloAtual}
        tipo={tipoAtual}
        enviaTicket={enviaTicket}
      />
      {showNewTicket ? (
        <ModalNovoTicket
          CardCode={task?.CardCode}
      ...
      {showEndTicket ? (
        <EndTicket
          onClose={() => handleCloseEndTicketModal()}
          ticketData={closingTicketData}
        />
      ) : (
        ""
      )}
      <div>
        <div className=" hidden flex flex-col customBorder shadow-md p-2 rounded-md   ">
          <div className="flex items-center gap-4">
            <h1 className="m-0 ml-2">Ações Rápidas</h1>
            <FaCircleInfo
              data-tooltip-id="tooltip-6"
              data-tooltip-content="As ações rápidas são requisições/informações repassados para internos através da agenda!"
            />
          </div>
          <div className=" flex justify-between w-2/3  mt-4 px-12 py-4 self-center   ">
            <div
              data-tooltip-id="tooltip-6"
              data-tooltip-content="Solicitar ao cac que agende uma visita para o cliente"
            >
              <BtnRapidas
                nome="Solicitar Visita com o cliente"
                icon={<IoIosChatbubbles size={25} />}
                onClick={() => handlePedirAcao("Agendar Visita", "data", "77")}
              />
            </div>
            <div className="hidden">
              <BtnRapidas
                bgColor="red-500"
                nome="Solicitar reunião com o cliente"
                icon={<GiMoneyStack size={30} />}
                onClick={() => handlePedirAcao("Agendar reunião", "data", "77")}
              />
              <BtnRapidas
                bgColor="amber-500"
                nome="Realizei Visita"
                icon={<MdConnectWithoutContact size={30} />}
                onClick={() =>
                  handlePedirAcao("Realizei Visita", "conteudo", "72")
                }
              />
              <BtnRapidas
                bgColor="blue-500"
                nome="Realizei Reunião"
                icon={<FaHandshake size={30} />}
                onClick={() =>
                  handlePedirAcao("Realizei Reunião", "conteudo", "73")
                }
              />
              <BtnRapidas
                bgColor="slate-500"
                nome="Tirar cliente da agenda"
                icon={<IoIosRemoveCircle size={30} />}
                onClick={() => handlePedirAcao("Retirar Cliente", "nada", "0")}
              />
            </div>
          </div>
        </div>
        <div className="  box-border">
          <div className=" flex justify-between mb-2 items-end">
            <GrnBtn
              nomeBtn="Novo Atendimento"
              onClick={() => adicionarNovoTicket()}
              icon={<IoMdAddCircle />}
              customCss="h-10"
            />
            <div className="w-4/12">
              <Input
                ...
                insidePlaceholder="Fulano de tal"
              />
            </div>
          </div>
          <div
            ...
            {loadingTickets ? (
              ...
              >
                ...
                <p className="font-semibold text-xl">
                  Esse cliente não possui tickets
                </p>
              </div>
            ) : (
