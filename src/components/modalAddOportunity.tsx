...
const schema = z.object({
  ...
  dataPrevista: z
    .string()
    .min(1, "Data de Prevista é obrigatória")
    .refine(isAfterToday, {
...
export function AddOportunity({
  ...
  async function onSubmit(data: FormData) {
    ...
    if (response.status == "success") {
      ...
      // Limpar parâmetros da URL antes do reload
      const url = new URL(window.location.href);
      url.searchParams.delete("openModal");
      url.searchParams.delete("cardCode");
  ...
  return (
    <div
      className={` ${
        isOpen ? "" : "hidden"
      } modalAdd w-screen h-screen bg-opacity-50 bg-black overflow-hidden  absolute flex items-center justify-center z-50 ${
        isOpen ? "show" : "enter"
      } `}
    >
      <div
        ...
        }   w-3/6 rounded-md shadow-md flex flex-col`}
      >
        <div className="w-full h-20 flex items-center justify-between  bg-white rounded-md">
          ...
          <div className="mr-6" onClick={() => handleClose()}>
            <WhiteBtn nomeBtn="Fechar" icon={<MdCancel />} />
          </div>
        </div>
        ...
        <form
          ...
          className="flex h-9/12 w-full px-8 py-4 box-border mb-4  justify-between gap-4"
        >
          <div className="flex flex-col w-1/2 h-full box-border">
            <Input
              customCss="h-1/2"
              insidePlaceholder="Título da oportunidade"
              placeholder="Título"
              name="titulo"
            ...
            <div>
              <Input
                id="codigoCliente"
                valueHandler={(e) => handleCodigoCliente(e)}
                insidePlaceholder="C000000 ou Nome do cliente"
                placeholder="Código do cliente"
                icon={<BsPersonBoundingBox />}
                name="codCliente"
                error={errors.codCliente?.message}
                register={register}
              ></Input>
              {warn ? (
                <span className="text-yellow-500 text-sm">
                  Cliente já possui uma oportunidade em andamento! (Apenas
                  informativo){" "}
                </span>
              ) : (
                ""
              ...
              <div className="relative z-40 ">
                <div
                  className={` box-border absolute z-50 w-80 flex flex-col bg-white shadow-md ${
                    clientes.length > 0 ? "customBorder" : ""
                  } mt-1 rounded-md`}
                >
                  {loadingSearch ? (
                    <div>
                    ...
                    clientes.map((e) => {
                      ...
                      return (
                        <div
                          onClick={() => handleClickFoundClient(e.codigo)}
                          key={e.codigo}
                          className={` cursor-pointer hover:bg-slate-100 bg-white py-1 px-2  flex items-center justify-center font-semibold transition-all duration-300 hover:bg-shadow-md `}
                        >
                          <p>
                            {e.codigo} - {e.nome}
                          </p>
            ...
            <div className="flex flex-col pb-6">
              ...
              <div className="flex">
                ...
                <div
                  ...
                  data-tooltip-content="Qualificação"
                  className="flex"
                  onClick={() => setEtapa(1)}
                >
                  <button
                    type="button"
                    className={`w-6 h-6 border-none rounded-sm ${
                      etapa >= 1 ? "bg-green-500" : ""
                    }`}
                  ></button>
                  <div
                    className={
                      etapa >= 1 ? `triangle-right` : `triangle-right-grey`
                    }
                  ></div>
                </div>
                <div
                  data-tooltip-id="tooltip-1"
                  data-tooltip-content="Agendamento / Reunião"
                  className="flex"
                  onClick={() => setEtapa(2)}
                >
                  {" "}
                  <div className="triangle-right-white absolute"></div>
                  <button
                    type="button"
                    className={`w-10 h-6 border-none rounded-sm ${
                      etapa >= 2 ? "bg-green-500" : ""
                    }`}
                  ></button>
                  <div
                    className={
                      etapa >= 2 ? `triangle-right` : `triangle-right-grey`
                    }
                  ></div>
                </div>
                <div
                  data-tooltip-id="tooltip-1"
                  data-tooltip-content="Diagnóstico"
                  className="flex"
                  onClick={() => setEtapa(3)}
                >
                  {" "}
                  ...
                  <button
                    type="button"
                    className={`w-10 h-6 border-none rounded-sm ${
                      etapa >= 3 ? "bg-green-500" : ""
                    }`}
                  ></button>
                  <div
                    className={
                      etapa >= 3 ? `triangle-right` : `triangle-right-grey`
                    }
                  ></div>
                </div>
                <div
                ...
                >
                  ...
                  <button
                    ...
                    className={`w-10 h-6 border-none ${
                      etapa >= 4 ? "bg-green-500" : ""
                    }`}
                  ></button>
                  <div
                    className={
                      etapa >= 4 ? `triangle-right` : `triangle-right-grey`
                    }
                  ></div>
                </div>
                <div
                  data-tooltip-id="tooltip-1"
                  data-tooltip-content="Proposta de Valor"
                  className="flex"
                  onClick={() => setEtapa(5)}
                >
                  {" "}
                  ...
                  <button
                    type="button"
                    className={`w-10 h-6 border-none ${
                      etapa >= 5 ? "bg-green-500" : ""
                    }`}
                  ></button>
                  <div
                    className={
                      etapa >= 5 ? `triangle-right` : `triangle-right-grey`
                    }
                  ></div>
                </div>
              </div>
            ...
            <Input
              type="date"
              placeholder="Data prevista de conclusão da etapa"
              customCss="mt-2"
              insidePlaceholder="DD/MM/YYYY"
              name="dataPrevista"
          ...
          <div className=" flex flex-col w-1/2">
            <div className="mt-6 h-full box-border w-full mb-4">
              ...
              <textarea
                ...
                value={quillValue}
                className={` p-2 box-border w-full h-full rounded-md customBorder shadow-md focus:outline-none `}
              />
            </div>
          </div>
        </form>
        ...
        <div className=" flex items-center justify-between h-20 w-full ">
          <button
            onClick={() => handleClose()}
            className=" hover:scale-105 ml-12 h-10 w-28 customRedBorder outline-none bg-white rounded-md font-semibold text-red-500 cursor-pointer hover:bg-red-500 hover:text-white transition-all duration-300 "
          >
            Cancelar
          </button>
          <button
            ...
            type="submit"
            className=" hover:scale-105 mr-12 h-10 w-28 rounded-md border-none bg-green-500 text-white font-semibold text-lg  cursor-pointer hover:bg-green-600 transition-all duration-300 shadow-md hover:shadow-lg flex gap-4 items-center justify-center"
          >
