    ...
    <>
      <Draggable draggableId={task.Id.toString()} key={task.Id} index={index}>
        {(provided: any, snapshot: any) => {
          return (
            <div
              data-tooltip-id={`${task.ClienteAtivo != "Y" ? "tooltip-1" : ""}`}
              data-tooltip-content="cliente inativo"
              className={`${
                task.ClienteAtivo != "Y" ? "opacity-40" : ""
              } z-40 hover:z-50 w-full relative h-auto px-2 scaleCustom py-2 rounded-md ${
            ...
            >
              <div className="w-11/12">
                ...
                <p className="m-0 text-xs font-semibold mt-3 flex gap-2 items-center">
                  {" "}
                  <CgProfile /> R$ {formatedPrice} <LuDot />{" "}
              ...
              >
                <BsExclamationCircleFill
                  ...
                  data-tooltip-content="Essa oportunidade foi destacada pelo seu gestor!"
                  size={22}
                  className={` ${
                    !isHighlighted
                      ? "opacity-0 fixed h-0 w-0 "
                      : "opacity-100 scale-105"
                  } z-50 corDestaque transition-all duration-300 p-1 `}
                />
                {isInDrag ? (
