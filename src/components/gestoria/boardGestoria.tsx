...
export const TaskContextGestoria = React.createContext(
  {} as taskContextGestoriaProps
);
interface task {
  Id: number;
  CardCode: string;
...
export function BoardGestoria() {
  ...
  const carregaTasksSAP = async (filter: string = "") => {
    const response = await ajax({
      method: "POST",
      endpoint: "/gestoria/tasks",
      data: { filter: filter, filtros: filters, gerenciados: gerenciados },
    });
    if (!response) {
      return false;
    }
