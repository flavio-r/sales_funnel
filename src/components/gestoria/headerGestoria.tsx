...
export function HeaderGestoria({
  setSearch,
  setFilters,
  setGerenciadosContext,
  setAllGerenciadosContext,
  Filters,
  Gerenciados,
}: header) {
  ...
  const carregaGerenciados = async () => {
    ...
    if (response.status == "success") {
      ...
      adicionaSelecionadoGerenciados(data, Gerenciados);
    }
  };
  ...
  const selectAll = () => {
    gerenciados.forEach((gerenciado) => {
      gerenciado.Selecionado = true;
    });
    setGerenciados([...gerenciados]);
  };
      ...
      <div className="headerHeight flex shadow-md w-full items-center box-border justify-between fixed top-0 left-0 right-0 z-50 bg-white">
          ...
          <div className="flex gap-4 mr-4 ml-4">
            ...
            <div className="flex flex-col items-center justify-center">
              ...
              <button className=" hover:scale-105 h-9 w-24 customRedBorder outline-none bg-white rounded-md font-semibold text-red-500 cursor-pointer hover:bg-red-500 hover:text-white transition-all duration-300 ">
                {indicadores.perdidos !== undefined ? (
        ...
        >
          ...
          <div
            ...
            className={` ${
              showVendors ? "" : "hidden"
            } customBorder mt-14 customListWidth rounded-md box-border absolute z-50 bg-white flex flex-col shadow-lg max-h-[min(85vh,24rem)] flex-nowrap overflow-hidden`}
          >
            <div className="w-full flex gap-4 py-2 items-center justify-center border-b border-black/10 shrink-0">
              ...
              />
              <GrnBtn
                customCss="bg-red-500 hover:bg-red-600"
                onClick={() => deSelectAll()}
                nomeBtn="Tirar todos"
                icon={<IoIosRemoveCircle />}
              />
            </div>
            <div className="overflow-y-auto overflow-x-hidden min-h-0 flex-1 max-h-[min(75vh,18rem)]">
              {gerenciados.map(
                (gerenciado: {
                  VendedorExterno: string;
                  CodigoVendedor: string;
            ...
            <GrnBtn
              ...
              customCss="mt-2 mb-2 w-11/12 self-center flex-shrink-0"
