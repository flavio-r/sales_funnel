...
export function ModalNovoTicket({ onClose, CardCode }: novoTicketProps) {
  ...
  return (
    ...
    >
          ...
          <div className="flex items-center gap-2">
            <label className="switch">
              <input
                ...
                id="switcher"
                checked={criaProximo}
                onClick={() => setCriaProximo(!criaProximo)}
              />
              <span className="slider"></span>
            </label>
            <label className="m-0 font-semibold text-lg" htmlFor="switcher">
              Criar próximo antendimento
            </label>
          </div>
          <GrnBtn
            form="formNovoTicket"
      ...
      <div
        onSubmit={handleSubmit(onSubmit)}
        className={`bg-white w-5/12 h-4/6 ${
          criaProximo ? "" : "hidden"
        }   rounded-md flex flex-col box-border shadow-lg p-2`}
      >
        <div className="flex justify-between px-4 items-center mt-2">
