...
class DeployPortainer {
  ...
  pullarImagemDockerHub = async () => {
    const config = {
      ...
      url:
        this.portainerUrl +
        "/endpoints/2/docker/images/create?fromImage=copapel/" +
        this.Imagem +
        "",
      headers: {
        "Content-Type": "application/json",
  ...
  pararContainerPorImagem = async () => {
    try {
      ...
      const targetContainer = containers.find(
        (container) => container.Image === "copapel/" + this.Imagem
      );
      ...
      if (targetContainer) {
        const stopContainerConfig = {
          ...
          url:
            this.portainerUrl +
            `/endpoints/2/docker/containers/${targetContainer.Id}/stop`,
          headers: {
            Authorization: `Bearer ${this.token}`,
          },
          httpsAgent: agent,
  ...
  rodarContainer = async () => {
    const config = {
      ...
      url:
        this.portainerUrl +
        `/endpoints/2/docker/containers/${this.idContainer}/start`,
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${this.token}`,
      },
